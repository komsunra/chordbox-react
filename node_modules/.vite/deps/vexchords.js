...
// node_modules/vexchords/chordbox.js
var ChordBox = class {
  ...
  drawText(x5, y5, msg, attrs2) {
    ...
    return text.move(x5 - text.length() / 2, y5);
  }
  drawLine(x5, y5, newX, newY) {
    return this.canvas.line(0, 0, newX - x5, newY - y5).move(x5, y5);
  }
  draw({
    chord,
    position: position2,
  ...
  lightUp({ string, fret, label }) {
    ...
    const x5 = this.x + this.spacing * stringNum;
    let y5 = this.y + this.fretSpacing * fretNum;
    if (fretNum === 0) {
      y5 -= this.metrics.bridgeStrokeWidth;
    }
    if (!mute) {
      this.canvas.circle().move(x5, y5 - this.fretSpacing / 2).radius(this.metrics.circleRadius).stroke({ color: this.params.strokeColor, width: this.params.strokeWidth }).fill(fretNum > 0 ? this.params.strokeColor : this.params.bgColor);
    } else {
      this.drawText(x5, y5 - this.fretSpacing, "X");
    }
    if (label) {
      const fontSize = this.metrics.fontSize * 0.55;
      const textYShift = fontSize * 0.66;
      this.drawText(x5, y5 - this.fretSpacing / 2 - textYShift, label, {
  ...
  lightBar(stringFrom, stringTo, theFretNum) {
    ...
    const x5 = this.x + this.spacing * stringFromNum - this.metrics.barShiftX;
    const xTo = this.x + this.spacing * stringToNum + this.metrics.barShiftX;
    const y5 = this.y + this.fretSpacing * (fretNum - 1) + this.fretSpacing / 4;
    const yTo = this.y + this.fretSpacing * (fretNum - 1) + this.fretSpacing / 4 * 3;
    this.canvas.rect(xTo - x5, yTo - y5).move(x5, y5).radius(this.metrics.barreRadius).fill(this.params.strokeColor);
    return this;
  }
};
var chordbox_default = ChordBox;

// node_modules/vexchords/builder.js
var SHAPES = {
  "M E": {
    name: "Maj",
    chord: [[3, 2], [4, 3], [5, 3]],
...
function build2(key, string, shape) {
  const theString = string.toUpperCase();
  const position2 = POSITIONS[theString][key];
...
// node_modules/vexchords/index.js
function draw(sel, chord, opts) {
  return new chordbox_default(sel, opts).draw(chord);
}
export {
  chordbox_default as ChordBox,
  POSITIONS,
  SHAPES,
  build2 as build,
  draw
};
//# sourceMappingURL=vexchords.js.map
